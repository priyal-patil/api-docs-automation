...
const TEST_CONTENT_TYPE_UID  = liveData.content_type_uid || 'ref_child_ct_93257';
const TEST_ENVIRONMENT       = liveData.environment       || 'apienv';
...
const TEST_TAXONOMY_UID      = liveData.taxonomy_uid      || process.env.CS_TAXONOMY_UID     || 'test';
const TEST_TERM_UID          = liveData.term_uid          || process.env.CS_TERM_UID         || 'test3m';
const TEST_GLOBAL_FIELD_UID  = liveData.global_field_uid  || process.env.CS_GLOBAL_FIELD_UID || 'seo_settings_43634';

// Modules that return 422 due to no published content on the test stack — warn instead of fail
const SKIP_422_MODULES = ['Entries', 'Entry Variants'];
...
const SKIP_ERROR_REQUESTS = new Set([
  'Sync using pagination token',  // sample pagination_token is always expired
  'Subsequent Sync',              // sample sync_token is always expired
  'Get a single asset',           // Try Out panel proxies the call; our api_key may not apply
...
// requestName alone can collide across modules — fileKey disambiguates the
// filename without changing requestName itself (the comparator matches on that field).
...
// ── Main test suite ───────────────────────────────────────────────────────────
test.describe('Phase 2 — Live Try Out Panel Tests', () => {
  ...
  const targets = scrapedRequests.length > 0
    ...
    : [
        { name: 'Content Types', module: 'content-types', url: 'https://www.contentstack.com/docs/developers/apis/content-delivery-api/content-types#get-all-content-types', method: 'GET' },
      ];

  // Request names can repeat across modules — Playwright requires unique test
  // titles, so qualify with the module whenever the name collides.
  ...
  for (const target of targets) {
    ...
    test(title, async ({ page }) => {
      ...
      await fillParamField(page, 'access_token', DELIVERY_TOKEN);
      await fillParamField(page, 'delivery_token', DELIVERY_TOKEN);
