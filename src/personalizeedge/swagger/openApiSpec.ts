...
/**
 * Personalize Edge has no Postman collection — same as Lytics/Personalize
 * Management, it publishes a live OpenAPI 3.0 spec instead. This fetches +
 * flattens it into the same PostmanRequest[] shape the other comparators
 * consume.
 */
export async function fetchPersonalizeEdgeOpenApiSpec(baseHost: string, cacheFilename = 'openapi-spec-personalizeedge.json'): Promise<any> {
  ...
  try {
    ...
    if (fs.existsSync(cachePath)) {
      console.warn('⚠️  Using cached Personalize Edge OpenAPI spec (live fetch failed)');
  ...
  for (const [urlPath, methods] of Object.entries<any>(spec.paths ?? {})) {
    for (const [method, op] of Object.entries<any>(methods)) {
      ...
      if (bodyExample !== undefined) {
        body = { mode: 'raw', raw: JSON.stringify(bodyExample) };
