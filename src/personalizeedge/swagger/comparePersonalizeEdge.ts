...
import { fetchPersonalizeEdgeOpenApiSpec, parseSwaggerRequests } from './openApiSpec';
...
/**
 * Three-way comparison for the Personalize Edge API — same shape as
...
export async function runComparisonPersonalizeEdge(): Promise<ComparisonResult[]> {
  ...
  for (const { doc, tryOut } of scraped) {
    ...
    if (swaggerReq) {
      ...
      // No auth headers at all for this public API — only x-project-uid is global/expected everywhere.
      const GLOBAL_HEADERS = new Set(['xprojectuid', 'contenttype']);
      ...
      mismatches.push({
        ...
        source: 'Doc → OpenAPI spec',
        detail: `No matching operation found in the Personalize Edge OpenAPI spec for "${doc.name}"`,
        severity: 'warning',
      });
    }

    // ── 2. Request body field comparison (POST/PUT/PATCH): OpenAPI schema ↔ doc's Sample Request ─
    if (doc.method === 'POST' || doc.method === 'PUT' || doc.method === 'PATCH') {
  ...
  const outPath = path.join(__dirname, '../../../reports/comparison-results-personalizeedge.json');
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));

  const pass = results.filter(r => r.status === 'pass').length;
  const warn = results.filter(r => r.status === 'warning').length;
  const fail = results.filter(r => r.status === 'fail').length;
  console.log(`\n📊  Comparison done (Personalize Edge) — ✅ ${pass} pass  ⚠️ ${warn} warning  ❌ ${fail} fail`);
...
runComparisonPersonalizeEdge().catch(console.error);
