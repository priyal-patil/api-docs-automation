...
const BASE = 'https://www.contentstack.com/docs/developers/apis/personalize-edge-api';

// Module slugs come from the rendered left nav — confirmed live, do NOT guess.
const MODULES = ['manifest', 'user-attributes', 'events'];

/**
 * Method badge + endpoint URL extraction — same DOM shape as every other
 * Developer Hub doc page. See scrapeAllLytics.ts for why the badge's first
 * <span> is read unconditionally rather than matched by class.
 */
...
async function extractJsonAfterMarker(page: Page, anchorId: string, marker: string): Promise<string | undefined> {
  return page.evaluate(({ id, marker }: { id: string; marker: string }): string | undefined => {
    ...
    for (let i = markerIdx + 1; i < scoped.length; i++) {
      const el = scoped[i];
      if ((el.tagName === 'PRE' || el.tagName === 'CODE') && (el.textContent?.includes('{') || el.textContent?.includes('['))) {
        try {
          const parsed = JSON.parse(el.textContent.trim());
          if (parsed && typeof parsed === 'object') return el.textContent.trim();
        } catch { /* not JSON */ }
      }
    }
    return undefined;
...
function extractKeys(raw: string | undefined): string[] | undefined {
  ...
  try {
    ...
    if (Array.isArray(parsed)) return parsed.length && typeof parsed[0] === 'object' ? Object.keys(parsed[0]) : undefined;
    if (parsed && typeof parsed === 'object') return Object.keys(parsed);
  } catch { /* not JSON */ }
  return undefined;
}

export interface ScrapedPersonalizeEdgeRequest {
  doc: DocRequest;
  tryOut: TryOutData & { responseBodyKeys?: string[] };
}

async function scrapeModule(browser: Awaited<ReturnType<typeof chromium.launch>>, module: string): Promise<ScrapedPersonalizeEdgeRequest[]> {
  const moduleUrl = `${BASE}/${module}`;
  const results: ScrapedPersonalizeEdgeRequest[] = [];
...
export async function scrapeAllPersonalizeEdge(): Promise<ScrapedPersonalizeEdgeRequest[]> {
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-personalizeedge.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));
  console.log(`\n✅  Scraped ${results.length} Personalize Edge requests → ${outPath}`);

  // Live Try Out "Send" button confirmed for this doc template (same as
  // Lytics/Personalize Management) — no synthetic baseline written here.
  // Run `npm run tryout:personalizeedge` to produce
  // reports/tryout-results-personalizeedge.json.

  return results;
}

scrapeAllPersonalizeEdge().catch(console.error);
