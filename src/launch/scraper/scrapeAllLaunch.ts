...
const BASE = 'https://www.contentstack.com/docs/developers/apis/launch-api';

// Module slugs come from the rendered left nav — confirmed live, do NOT guess.
const MODULES = ['projects', 'environments', 'file-upload', 'deployments', 'deployment-logs', 'launch-product-analytics', 'server-logs'];
...
export interface ScrapedLaunchRequest {
  doc: DocRequest;
  tryOut: TryOutData & { responseBodyKeys?: string[] };
}

async function scrapeModule(browser: Awaited<ReturnType<typeof chromium.launch>>, module: string): Promise<ScrapedLaunchRequest[]> {
  const moduleUrl = `${BASE}/${module}`;
  const results: ScrapedLaunchRequest[] = [];
...
export async function scrapeAllLaunch(): Promise<ScrapedLaunchRequest[]> {
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-launch.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));
  console.log(`\n✅  Scraped ${results.length} Launch requests → ${outPath}`);

  // Live Try Out "Send" button expected (same doc template as Lytics/
  // Personalize) — no synthetic baseline written here. Run
  // `npm run tryout:launch` to produce reports/tryout-results-launch.json.

  return results;
}

scrapeAllLaunch().catch(console.error);
