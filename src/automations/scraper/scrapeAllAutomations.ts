...
const BASE = 'https://www.contentstack.com/docs/developers/apis/automations-management-api';

// Module slugs come from the rendered left nav — confirmed live, do NOT guess.
const MODULES = ['projects', 'automations', 'execution-logs', 'audit-logs', 'project-variables', 'accounts'];

const HEADER_KEYS = new Set(['authtoken', 'organization_uid', 'content-type']);

export interface ScrapedAutomationsRequest {
  doc: DocRequest;
  tryOut: TryOutData & { responseBodyKeys?: string[] };
}

/**
 * Method badge + endpoint URL live in ONE row that is the next sibling of the
 * heading's OWN "flex items-center justify-between" wrapper — NOT a sibling
 * of the anchor div itself (confirmed live: the id lives on an inner div;
 * walking ITS siblings only reaches the copy-link button, never this row).
 * The badge's class scheme also differs per method — confirmed live: GET/POST/PUT
 * use "bg-docs-*" utility classes, but DELETE uses "bg-[var(--Surfaces-Docs-
 * Extended-Red-Soft)]" instead, so matching on a "bg-docs-" substring silently
 * missed every DELETE badge. Read the row's first <span> unconditionally instead.
 */
...
/** Bound this request's own section: from its heading up to (but not including) the next heading — same approach as the CMA scraper. */
function sectionScopeScript() {
  return `
    function sectionElements(anchorId) {
      const all = Array.from(document.querySelectorAll('body *'));
      const anchorEl = document.getElementById(anchorId);
      if (!anchorEl) return all;
      const startIdx = all.indexOf(anchorEl);
      if (startIdx === -1) return all;
      const ownHeading = anchorEl.querySelector('h1,h2,h3,h4,h5,h6');
...
/**
 * Finds the JSON code block immediately following a text marker (e.g. "Sample
 * Request", "Sample Response"), scoped to this request's own section, and
 * returns the RAW JSON text — callers parse it themselves (matching the CDA/CMA
 * convention where tryOut.bodyContent holds raw text, not pre-extracted keys).
 */
...
async function scrapeModule(browser: Awaited<ReturnType<typeof chromium.launch>>, module: string): Promise<ScrapedAutomationsRequest[]> {
  const moduleUrl = `${BASE}/${module}`;
  const results: ScrapedAutomationsRequest[] = [];
  const page = await browser.newPage();
  console.log(`\n📂  Module: ${module}`);

  try {
    // Anchor discovery is occasionally flaky on first load (confirmed live —
    // same module found 0 anchors once, 5/5 the very next run with nothing
    // else changed) — same class of client-render timing issue the CMA
    // scraper already retries around. Retry a couple of times before giving up.
    ...
    for (const { href, text } of anchors) {
      ...
      // No live Try Out execution exists for this API (no Send button) — same
      // as Analytics. Try Out fields mirror the doc's own static param list;
      // bodyContent holds the RAW JSON text (matching the CDA/CMA convention —
      // compare*.ts's extractKeys() parses this itself, it is not pre-parsed here).
...
export async function scrapeAllAutomations(): Promise<ScrapedAutomationsRequest[]> {
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-automations.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));
  console.log(`\n✅  Scraped ${results.length} Automations requests → ${outPath}`);

  // Synthetic Try Out baseline — see scrapeAllAnalytics.ts for why: no live
  // Send button exists, so the doc's static Sample Response stands in as the
  // comparison baseline for the response-body check against live Newman output.
  const tryOutResults: TryOutTestResult[] = results.map(({ doc, tryOut }) => ({
    ...
    passed: true,
    flags: ['ℹ️  Automations Management API docs have no Try Out "Send" button — static doc Sample Response used as baseline'],
  }));
  const tryOutPath = path.join(__dirname, '../../../reports/tryout-results-automations.json');
...
scrapeAllAutomations().catch(console.error);
