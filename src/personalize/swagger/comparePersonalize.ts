...
import { fetchPersonalizeOpenApiSpec, parseSwaggerRequests } from './openApiSpec';
...
/**
 * Three-way comparison for the Personalize Management API — same shape as
 * compareLytics.ts, with the OpenAPI spec (fetched live from /openapi)
 * standing in for a Postman collection, since Personalize has none:
 *   A = Doc description (scraped params/headers/request body)
 *   B = Doc's live Try Out result (real Send button, same as Lytics)
 *   C = OpenAPI spec definition + live Swagger execution results
 *       (runSwaggerPersonalize.ts, output shape reuses NewmanResult unchanged)
 *
 * Checks:
 ...
 *  4. Execution failures (live request returned 4xx/5xx) — genuine business-rule
 *     responses (e.g. "only one draft version allowed") are still surfaced
 *     honestly, same philosophy as compareLytics.ts's quota-error handling.
 *  5. Coverage: endpoints in the spec but missing from the docs, and vice versa
 */
export async function runComparisonPersonalize(): Promise<ComparisonResult[]> {
  const scrapedPath = path.join(__dirname, '../../../reports/scraped-requests-personalize.json');
  if (!fs.existsSync(scrapedPath)) {
    throw new Error('scraped-requests-personalize.json not found — run `npm run scrape:personalize` first');
  }
  const scraped: Array<{ doc: DocRequest; tryOut: TryOutData }> = JSON.parse(fs.readFileSync(scrapedPath, 'utf-8'));

  const tryOutResultsPath = path.join(__dirname, '../../../reports/tryout-results-personalize.json');
  const tryOutResults: TryOutTestResult[] = fs.existsSync(tryOutResultsPath)
    ? JSON.parse(fs.readFileSync(tryOutResultsPath, 'utf-8'))
    : [];

  const newmanResultsPath = path.join(__dirname, '../../../reports/newman-results-personalize.json');
  const newmanResults: NewmanResult[] = fs.existsSync(newmanResultsPath)
    ? JSON.parse(fs.readFileSync(newmanResultsPath, 'utf-8'))
    : [];

  const spec = await fetchPersonalizeOpenApiSpec(BASE_HOST);
  ...
  for (const { doc, tryOut } of scraped) {
    ...
    if (swaggerReq) {
      ...
      // Path variables (e.g. {uid}, {versionUid}) live in the URL template, not as query params
      ...
      const GLOBAL_HEADERS = new Set(['authtoken', 'authorization', 'xprojectuid', 'contenttype']);
      ...
      mismatches.push({
        ...
        source: 'Doc → OpenAPI spec',
        detail: `No matching operation found in the Personalize OpenAPI spec for "${doc.name}"`,
        severity: 'warning',
      });
    }
    ...
    const executionHadNoTestData = !!newmanResult?.error && (
      newmanResult.error.includes('was created') || newmanResult.error.includes('did not return a uid')
    );
  ...
  const outPath = path.join(__dirname, '../../../reports/comparison-results-personalize.json');
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));

  const pass = results.filter(r => r.status === 'pass').length;
  const warn = results.filter(r => r.status === 'warning').length;
  const fail = results.filter(r => r.status === 'fail').length;
  console.log(`\n📊  Comparison done (Personalize) — ✅ ${pass} pass  ⚠️ ${warn} warning  ❌ ${fail} fail`);
...
runComparisonPersonalize().catch(console.error);
