...
const BASE = 'https://www.contentstack.com/docs/developers/apis/personalize-management-api';

// Module slugs come from the rendered left nav — confirmed live, do NOT guess.
const MODULES = ['attributes', 'audiences', 'experiences', 'events', 'experiences-priority', 'experience-analytics', 'geolocation'];

/**
 * Method badge + endpoint URL extraction — same DOM shape as every other
 * Developer Hub doc page (Lytics/Automations/etc.). See scrapeAllLytics.ts
 * for why the badge's first <span> is read unconditionally rather than
 * matched by class (DELETE uses a different background-color class).
 */
...
export interface ScrapedPersonalizeRequest {
  doc: DocRequest;
  tryOut: TryOutData & { responseBodyKeys?: string[] };
}

async function scrapeModule(browser: Awaited<ReturnType<typeof chromium.launch>>, module: string): Promise<ScrapedPersonalizeRequest[]> {
  const moduleUrl = `${BASE}/${module}`;
  const results: ScrapedPersonalizeRequest[] = [];
...
export async function scrapeAllPersonalize(): Promise<ScrapedPersonalizeRequest[]> {
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-personalize.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));
  console.log(`\n✅  Scraped ${results.length} Personalize requests → ${outPath}`);

  // Live Try Out "Send" button confirmed for this doc template (same as
  // Lytics) — no synthetic baseline written here. Run `npm run tryout:personalize`
  // (tests/personalize/tryout-personalize.spec.ts) to produce
  // reports/tryout-results-personalize.json.

  return results;
}

scrapeAllPersonalize().catch(console.error);
