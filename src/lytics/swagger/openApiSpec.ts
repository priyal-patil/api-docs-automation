...
/**
 * Lytics has no Postman collection — it publishes a live OpenAPI 3.0 spec
 * instead (confirmed: GET /openapi returns real JSON, not a 404 stub). This
 * fetches + flattens it into the same PostmanRequest[] shape the other
 * comparators already consume, so compareLytics.ts can reuse the identical
 * matching/diff logic as compareAutomations.ts — only the SOURCE of the
 * "collection" side changes.
 */
export async function fetchLyticsOpenApiSpec(baseHost: string, cacheFilename = 'openapi-spec-lytics.json'): Promise<any> {
  ...
  try {
    ...
    if (fs.existsSync(cachePath)) {
      console.warn('⚠️  Using cached Lytics OpenAPI spec (live fetch failed)');
...
/** Flattens every path/method in the spec into PostmanRequest[], matched to the doc's own "summary" field (confirmed to match the doc's request names exactly, e.g. "Create a project"). */
  ...
  for (const [urlPath, methods] of Object.entries<any>(spec.paths ?? {})) {
    for (const [method, op] of Object.entries<any>(methods)) {
      ...
      requests.push({
        ...
        url: `${urlPath}`, // path template, e.g. /projects/{id} — {id}/{userUid} treated as path vars, same as Postman {{var}} handling
        params: queryParams,
