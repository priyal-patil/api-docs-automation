...
const BASE = 'https://www.contentstack.com/docs/developers/apis/knowledge-vault-api';

// Only one module page, three endpoints (Ingest/Update/Delete — no Get/List
// endpoint exists) — confirmed live via the rendered nav.
const MODULES = ['knowledge-vault'];

export interface ScrapedKnowledgeVaultRequest {
  doc: DocRequest;
  tryOut: TryOutData & { responseBodyKeys?: string[] };
}

/** Same doc framework as Automations/Brand Kit/GenAI — method+endpoint row is the next sibling of the heading wrapper. */
 ...
 * this request's own section, and returns the RAW text — callers parse it
 * themselves (matching the convention used across the other no-Try-Out modules).
 */
...
async function scrapeModule(browser: Awaited<ReturnType<typeof chromium.launch>>, module: string): Promise<ScrapedKnowledgeVaultRequest[]> {
  const moduleUrl = `${BASE}/${module}`;
  const results: ScrapedKnowledgeVaultRequest[] = [];
...
export async function scrapeAllKnowledgeVault(): Promise<ScrapedKnowledgeVaultRequest[]> {
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-knowledgevault.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));
  console.log(`\n✅  Scraped ${results.length} Knowledge Vault requests → ${outPath}`);
  ...
  const tryOutResults: TryOutTestResult[] = results.map(({ doc, tryOut }) => ({
    ...
    passed: true,
    flags: ['ℹ️  Knowledge Vault API docs have no Try Out "Send" button — static doc Sample Response used as baseline'],
  }));
  const tryOutPath = path.join(__dirname, '../../../reports/tryout-results-knowledgevault.json');
...
scrapeAllKnowledgeVault().catch(console.error);
