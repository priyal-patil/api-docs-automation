...
const BASE = 'https://www.contentstack.com/docs/developers/apis/brand-kit-management-api';

// Module slugs come from the rendered left nav — confirmed live, do NOT guess.
const MODULES = ['brand-kit', 'voice-profile', 'custom-credentials-llm-configuration'];

export interface ScrapedBrandKitRequest {
  doc: DocRequest;
 ...
 * heading's OWN "flex items-center justify-between" wrapper — same structure
 * confirmed on the Automations Management API docs (same doc framework).
 * DELETE badges use a different CSS class scheme than GET/POST/PUT, so read
 * the row's first <span> unconditionally rather than matching a class.
 */
 ...
 * returns the RAW JSON text — callers parse it themselves.
 */
...
async function scrapeModule(browser: Awaited<ReturnType<typeof chromium.launch>>, module: string): Promise<ScrapedBrandKitRequest[]> {
  const moduleUrl = `${BASE}/${module}`;
  const results: ScrapedBrandKitRequest[] = [];
  const page = await browser.newPage();
  console.log(`\n📂  Module: ${module}`);

  try {
    // Anchor discovery is occasionally flaky on first load (confirmed on the
    // Automations Management API scraper — same doc framework) — retry a
    ...
    for (const { href, text } of anchors) {
      ...
      // as Analytics/Automations. Try Out fields mirror the doc's own static
      // param list; bodyContent holds the RAW JSON text (compare*.ts parses it).
...
export async function scrapeAllBrandKit(): Promise<ScrapedBrandKitRequest[]> {
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-brandkit.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));
  console.log(`\n✅  Scraped ${results.length} Brand Kit requests → ${outPath}`);

  // Synthetic Try Out baseline — no live Send button exists, so the doc's
  // static Sample Response stands in as the comparison baseline for the
  // response-body check against live Newman output.
  const tryOutResults: TryOutTestResult[] = results.map(({ doc, tryOut }) => ({
    ...
    passed: true,
    flags: ['ℹ️  Brand Kit Management API docs have no Try Out "Send" button — static doc Sample Response used as baseline'],
  }));
  const tryOutPath = path.join(__dirname, '../../../reports/tryout-results-brandkit.json');
...
scrapeAllBrandKit().catch(console.error);
