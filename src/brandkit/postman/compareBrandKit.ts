...
/**
 * Three-way comparison for Brand Kit Management API (no live Try Out
 * execution exists, same as Analytics/Automations):
 ...
 */
export async function runComparisonBrandKit(): Promise<ComparisonResult[]> {
  const scrapedPath = path.join(__dirname, '../../../reports/scraped-requests-brandkit.json');
  if (!fs.existsSync(scrapedPath)) {
    throw new Error('scraped-requests-brandkit.json not found — run `npm run scrape:brandkit` first');
  }
  const scraped: Array<{ doc: DocRequest; tryOut: TryOutData }> = JSON.parse(fs.readFileSync(scrapedPath, 'utf-8'));

  const tryOutResultsPath = path.join(__dirname, '../../../reports/tryout-results-brandkit.json');
  const tryOutResults: TryOutTestResult[] = fs.existsSync(tryOutResultsPath)
    ? JSON.parse(fs.readFileSync(tryOutResultsPath, 'utf-8'))
    : [];

  const newmanResultsPath = path.join(__dirname, '../../../reports/newman-results-brandkit.json');
  ...
  const cachedPostman = path.join(__dirname, '../../../reports/postman-collection-brandkit.json');
  try {
    postmanRequests = await fetchPostmanCollection(POSTMAN_BRANDKIT_COLLECTION_ID, 'postman-collection-brandkit.json');
  } catch {
    if (fs.existsSync(cachedPostman)) {
      console.warn('⚠️  Using cached Brand Kit Postman collection (live fetch failed)');
      postmanRequests = JSON.parse(fs.readFileSync(cachedPostman, 'utf-8'));
    } else {
      throw new Error('Postman collection unavailable — check POSTMAN_API_KEY and POSTMAN_BRANDKIT_COLLECTION_ID');
    }
  }
  ...
  for (const { doc, tryOut } of scraped) {
    ...
    if (postmanReq) {
      ...
      // Path variables (e.g. {{brand_kit_uid}}) live in the URL, not as query params
      ...
      const GLOBAL_HEADERS = new Set(['authtoken', 'organizationuid', 'contenttype', 'authorization', 'apikey']);
      ...
      mismatches.push({
        ...
        source: 'Doc → Postman',
        detail: `No matching request found in the Brand Kit Postman collection for "${doc.name}"`,
        severity: 'warning',
      });
    }
  ...
  const outPath = path.join(__dirname, '../../../reports/comparison-results-brandkit.json');
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));

  const pass = results.filter(r => r.status === 'pass').length;
  const warn = results.filter(r => r.status === 'warning').length;
  const fail = results.filter(r => r.status === 'fail').length;
  console.log(`\n📊  Comparison done (Brand Kit) — ✅ ${pass} pass  ⚠️ ${warn} warning  ❌ ${fail} fail`);
...
runComparisonBrandKit().catch(console.error);
