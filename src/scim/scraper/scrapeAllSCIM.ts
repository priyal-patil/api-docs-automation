...
const BASE = 'https://www.contentstack.com/docs/developers/apis/scim-api';

// Module slugs come from the rendered nav on the SCIM API docs index — do NOT
// guess slugs. "Introduction" has no runnable request examples, so it's
...
const MODULES: Record<string, { skipAnchors: string[] }> = {
  ...
  'groups':           { skipAnchors: ['#authentication'] },
  'schema-discovery': { skipAnchors: ['#authentication'] },
};

// Headers documented globally across SCIM API requests, confirmed live via the
// "Open Builder" panel's data-param-key inputs — unlike Administration API,
// organization_uid/user_id/group_id are URL path parameters here, not headers.
const HEADER_KEYS = new Set(['authorization', 'content-type']);
...
async function extractMethodAndEndpoint(page: Page, anchorId: string): Promise<{ method: DocRequest['method']; endpoint: string }> {
  return page.evaluate((id: string) => {
    ...
    // This doc page renders up to 4 duplicate DOM copies of each section (confirmed
    // live on "Replace Users in Group" — responsive mobile/desktop layout variants),
    // ALL of which have their own <h2>, unlike CMA where usually only one candidate
    // did. Picking merely "the first one with an h2" can land on a hidden duplicate,
    // silently returning empty params/headers for that request. Must prefer a
    // candidate that is BOTH visible (offsetParent !== null) AND has an h2.
...
async function extractParams(page: Page, anchorId: string): Promise<DocParam[]> {
  return page.evaluate((id: string): DocParam[] => {
    ...
    const headerKeysInline = new Set(['authorization', 'content-type']);
...
async function extractHeaders(page: Page, anchorId: string): Promise<DocHeader[]> {
  return page.evaluate((id: string): DocHeader[] => {
    ...
    const anchorEl = idCandidates.length > 1
      ? (idCandidates.find(el => el.querySelector('h2') && (el as HTMLElement).offsetParent !== null) ?? idCandidates.find(el => el.querySelector('h2')) ?? idCandidates[idCandidates.length - 1])
      : idCandidates[0];
    let scoped: Element[] = all;
    if (anchorEl) {
      const startIdx = all.indexOf(anchorEl);
...
export async function scrapeAllSCIM(): Promise<ScrapedRequest[]> {
  const CONCURRENCY = 4;
  ...
  const outPath = path.join(__dirname, '../../../reports/scraped-requests-scim.json');
...
scrapeAllSCIM().catch(console.error);
