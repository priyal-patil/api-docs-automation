...
/**
 * Two-way comparison for the SCIM API:
...
export async function runComparisonSCIM(): Promise<ComparisonResult[]> {
  const scrapedPath = path.join(__dirname, '../../reports/scraped-requests-scim.json');
  if (!fs.existsSync(scrapedPath)) {
    throw new Error('scraped-requests-scim.json not found — run `npm run scrape:scim` first');
  }
  ...
  const outPath = path.join(__dirname, '../../reports/comparison-results-scim.json');
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));

  const pass = results.filter(r => r.status === 'pass').length;
  const warn = results.filter(r => r.status === 'warning').length;
  const fail = results.filter(r => r.status === 'fail').length;
  console.log(`\n📊  Comparison done (SCIM) — ✅ ${pass} pass  ⚠️ ${warn} warning  ❌ ${fail} fail`);
...
runComparisonSCIM().catch(console.error);
